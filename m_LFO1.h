// --------------------------------------------------------------------------
// This file is part of the KAGOUYAR firmware.
//
//    KAGOUYAR firmware is free software: you can redistribute it and/or modify
//    it under the terms of the GNU General Public License as published by
//    the Free Software Foundation, either version 3 of the License, or
//    (at your option) any later version.
//
//    KAGOUYAR firmware is distributed in the hope that it will be useful,
//    but WITHOUT ANY WARRANTY; without even the implied warranty of
//    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
//    GNU General Public License for more details.
//
//    You should have received a copy of the GNU General Public License
//    along with KAGOUYAR firmware. If not, see <http://www.gnu.org/licenses/>.
// --------------------------------------------------------------------------

double g_LFO1_noise[2]; // pour le noise avec interpolation cubic
double g_LFO1_AR[nb_voice+1];
double g_phase_LFO1_div;
uint32_t g_LFO1_last_step, g_LFO1_reset;

inline void LFO1(double fq, double mix_factor, double increment) {
    double modulation = 0.;

    if (curent_config.c_LFO1_WF == WF_AR) {
		double const A_time = ADSR_time2ilter(fq);
		double const D_time = ADSR_time2ilter(mix_factor);
        for (uint32_t j=nb_voice; j--;) {
            double const tmp = allvoice[j].v_GATE >= 1.;
            double const time = tmp?A_time : D_time;
            _fonepole(g_LFO1_AR[j], tmp, time);
            modulation += g_LFO1_AR[j];
        }
        modulation = nb_voice;
        g_LFO1_AR[nb_voice] = modulation;
        g_Modulation[LFO1_OUT] = modulation;
        g_Modulation_Phase[LFO1_OUT] = modulation;
        g_Modulation_Reset[LFO1_OUT] = g_new_gate; // signal provenant de la fct "add_voice"
        g_new_gate = 0;

    } else {
        double WF1, phase = 0., overflow_phase;
        uint32_t source_addresse = curent_config.c_Modulation_Source[LFO1_MOD];
        switch (curent_config.c_Modulation_Type[LFO1_MOD]) {
        case LFO_Mix : // ok
            {
                if (source_addresse != NONE_OUT) { // pas de modulation
                    WF1 = g_Modulation[source_addresse];
                } else {
                    WF1 = 1.;
                }
                phase = g_Modulation_Phase[LFO1_OUT] + increment;                               // calcul de la phase
                overflow_phase = (int)phase;
                phase -= overflow_phase;
                g_Modulation_Reset[LFO1_OUT] = overflow_phase;
                g_Modulation_Phase[LFO1_OUT] = phase;

                double WF = LFO_compute_WF(phase, curent_config.c_LFO1_WF, g_LFO1_noise, g_Modulation_Reset[LFO1_OUT]);
                modulation = mix(WF, WF1, mix_factor);
            }
            break;
        case LFO_AM : // ok
            {
                if (source_addresse != NONE_OUT) { //automodulation
                    WF1 = g_Modulation[source_addresse];
                } else {
                    WF1 = 0.;
                }
                phase = g_Modulation_Phase[LFO1_OUT] + increment;                               // calcul de la phase
                overflow_phase = (int)phase;
                phase -= overflow_phase;
                g_Modulation_Reset[LFO1_OUT] = overflow_phase;
                g_Modulation_Phase[LFO1_OUT] = phase;

                double WF = LFO_compute_WF(phase, curent_config.c_LFO1_WF, g_LFO1_noise, g_Modulation_Reset[LFO1_OUT]);
                modulation = WF * (1. - (WF1+1.) * mix_factor);
            }
            break;
        case LFO_FM : // ok
            {
                WF1 = g_Modulation[source_addresse];
                double l_increment = increment * CV2req( -36.3763 + WF1 * mix_factor * 50.);

                phase = g_Modulation_Phase[LFO1_OUT] + l_increment;                               // calcul de la phase
                overflow_phase = _floor(phase);
                phase -= overflow_phase;
                g_Modulation_Phase[LFO1_OUT] = phase;
                g_Modulation_Reset[LFO1_OUT] = overflow_phase != 0.;
                modulation = LFO_compute_WF(phase, curent_config.c_LFO1_WF, g_LFO1_noise, g_Modulation_Reset[LFO1_OUT]);
            }
            break;
        case LFO_PM : // ok
            {
                WF1 = g_Modulation[source_addresse];
                phase = g_Modulation_Phase[LFO1_OUT] + increment;                               // calcul de la phase
                overflow_phase = (int)phase;
                phase -= overflow_phase;
                g_Modulation_Reset[LFO1_OUT] = overflow_phase;
                g_Modulation_Phase[LFO1_OUT] = phase;

                phase = wrap2(phase + 2. * WF1 * mix_factor);
                double WF = LFO_compute_WF(phase, curent_config.c_LFO1_WF, g_LFO1_noise, g_Modulation_Reset[LFO1_OUT]);
                modulation = WF;
            }
            break;
        case LFO_CLIP : // OK
           {
                //if (source_addresse != LFO1_OUT) {
                    WF1 = g_Modulation[source_addresse];
                //} else {//automodulation
                //    WF1 = 0.;
                //}

                phase = g_Modulation_Phase[LFO1_OUT] + increment;                               // calcul de la phase
                overflow_phase = (int)phase;
                phase -= overflow_phase;
                g_Modulation_Reset[LFO1_OUT] = overflow_phase;
                g_Modulation_Phase[LFO1_OUT] = phase;

                double WF = LFO_compute_WF(phase, curent_config.c_LFO1_WF, g_LFO1_noise, g_Modulation_Reset[LFO1_OUT]);

                modulation = _fmin(WF, WF1+2.*(1.-mix_factor));
                modulation = _fmax(modulation, WF1-2.*(1.-mix_factor));
            }
            break;
        case LFO_Fold : // fold : OK
            {
                if (source_addresse != NONE_OUT) { //pas de modulation
                    WF1 = g_Modulation[source_addresse];
                } else {
                    WF1 = 1.;
                }
                phase = g_Modulation_Phase[LFO1_OUT] + increment;                               // calcul de la phase
                overflow_phase = (int)phase;
                phase -= overflow_phase;
                g_Modulation_Reset[LFO1_OUT] = overflow_phase;
                g_Modulation_Phase[LFO1_OUT] = phase;

                double WF = LFO_compute_WF(phase, curent_config.c_LFO1_WF, g_LFO1_noise, g_Modulation_Reset[LFO1_OUT]);

                WF *= 1. + (WF1+1.)* mix_factor; // gain from 1 to 3
                WF = (WF >  1.)?  2. - WF : WF; // fold
                WF = (WF < -1.)? -2. - WF : WF;
                modulation = WF;
            }
            break;
        case LFO_Xor : // OK
            {
                if (source_addresse != NONE_OUT) { //automodulation
                    WF1 = g_Modulation[source_addresse];
                } else {
                    WF1 = 1.;
                }
                phase = g_Modulation_Phase[LFO1_OUT] + increment;                               // calcul de la phase
                overflow_phase = (int)phase;
                phase -= overflow_phase;
                g_Modulation_Reset[LFO1_OUT] = overflow_phase;
                g_Modulation_Phase[LFO1_OUT] = phase;

                double WF = LFO_compute_WF(phase, curent_config.c_LFO1_WF, g_LFO1_noise, g_Modulation_Reset[LFO1_OUT]);

                uint32_t i_WF1 = (WF1+1.) * 2147483648.;
                uint32_t i_WF2 = (WF+1.)  * 2147483648.;
                double tmp = mix_factor * (mix_factor+1.) * 7.9;
                uint32_t ui_tmp = tmp;                                                           //0 a 15 (0b1111)
                ui_tmp <<= 28;
                i_WF1 &= ui_tmp;
                i_WF2 ^= i_WF1;                                                         // on inverse les bits de poids fort en premier
                modulation =  (i_WF2 * int2double) - 1.;
            }
            break;
        case LFO_FILTER :
            {
                double filter_fq;
                if (source_addresse != NONE_OUT) {
                    filter_fq = mix(127., 127. * g_Modulation[source_addresse], mix_factor);
                } else {//automodulation
                    filter_fq = (0.5-mix_factor) * 254.;
                }
                phase = g_Modulation_Phase[LFO1_OUT] + increment;                               // calcul de la phase
                overflow_phase = (int)phase;
                phase -= overflow_phase;
                g_Modulation_Reset[LFO1_OUT] = overflow_phase;
                g_Modulation_Phase[LFO1_OUT] = phase;

                filter_fq = CV2req(filter_fq)*(1./13000.);
                double WF = LFO_compute_WF(phase, curent_config.c_LFO1_WF, g_LFO1_noise, g_Modulation_Reset[LFO1_OUT]);
                modulation = g_Modulation[LFO1_OUT];
                _fonepole(modulation, WF, filter_fq);
            }
            break;
        case LFO_RESET : // OK
            {
                uint32_t WF1_reset;
                double l_mix_factor;
                if ( (source_addresse != NONE_OUT) || (source_addresse != LFO1_OUT) ) { //automodulation
                    WF1_reset = g_Modulation_Reset[source_addresse];
                    l_mix_factor = mix_factor;
                } else {
                    if (g_LFO1_reset == 0) {
                        if (mix_factor > 0.51) {
                            WF1_reset = 1;
                            g_LFO1_reset = 1;
                        } else {
                            WF1_reset = 1;
                        }
                    } else {
                        WF1_reset = 0;
                        if (mix_factor < 0.49) {
                            g_LFO1_reset = 0;
                        }
                    }
                    l_mix_factor = 1.;
                }
                phase = g_Modulation_Phase[LFO1_OUT] + increment;                               // calcul de la phase
                overflow_phase = (int)phase;
                phase -= overflow_phase;
                g_Modulation_Reset[LFO1_OUT] = overflow_phase;
                if(WF1_reset) { phase = wrap(mix(wrap(phase+0.5), 0.5, l_mix_factor) + 0.5); }
                g_Modulation_Phase[LFO1_OUT] = phase;
                modulation = LFO_compute_WF(phase, curent_config.c_LFO1_WF, g_LFO1_noise, g_Modulation_Reset[LFO1_OUT]);
            }
            break;
        case LFO_SYNC :
            if (source_addresse != NONE_OUT) { // clock divid/ mult : FQ : frequency divider or multiplier; MOD: add an offset to the input phase.
                // strange result en automod
                double tmp = fq * 8.99;
                switch ((uint32_t) tmp) {
                    case 0 : // div 8
                        g_Modulation_Reset[LFO1_OUT] = 0;
                        if (g_Modulation_Reset[source_addresse]) {
                            g_phase_LFO1_div++;
                            if ( g_phase_LFO1_div >= 8) {
                                g_phase_LFO1_div = 0;
                                g_Modulation_Reset[LFO1_OUT] = 1;
                            }
                        }
                        phase = ( g_phase_LFO1_div + g_Modulation_Phase[source_addresse]) * 0.125;
                    break;
                    case 1 : // div 4
                        g_Modulation_Reset[LFO1_OUT] = 0;
                        if (g_Modulation_Reset[source_addresse]) {
                            g_phase_LFO1_div++;
                            if ( g_phase_LFO1_div >= 4) {
                                g_phase_LFO1_div = 0;
                                g_Modulation_Reset[LFO1_OUT] = 1;
                            }
                        }
                        phase = ( g_phase_LFO1_div + g_Modulation_Phase[source_addresse])*0.25;
                    break;
                    case 2 : // div 3
                        g_Modulation_Reset[LFO1_OUT] = 0;
                        if (g_Modulation_Reset[source_addresse]) {
                            g_phase_LFO1_div++;
                            if ( g_phase_LFO1_div >= 3) {
                                g_phase_LFO1_div = 0;
                                g_Modulation_Reset[LFO1_OUT] = 1;
                            }
                        }
                        phase = ( g_phase_LFO1_div + g_Modulation_Phase[source_addresse])/3.0;
                    break;
                    case 3 : // div 2
                        g_Modulation_Reset[LFO1_OUT] = 0;
                        if (g_Modulation_Reset[source_addresse]) {
                            g_phase_LFO1_div++;
                            if ( g_phase_LFO1_div >= 2) {
                                g_phase_LFO1_div = 0;
                                g_Modulation_Reset[LFO1_OUT] = 1;
                            }
                        }
                        phase = ( g_phase_LFO1_div + g_Modulation_Phase[source_addresse])*0.5;
                    break;
                    case 4 : // Id
                        g_Modulation_Reset[LFO1_OUT] = g_Modulation_Reset[source_addresse];
                        phase = g_Modulation_Phase[source_addresse];
                    break;
                    case 5 : // mult 2
                        phase = g_Modulation_Phase[source_addresse] * 2.;
                        g_Modulation_Reset[LFO1_OUT] = (phase < g_Modulation_Phase[source_addresse]);
                    break;
                    case 6 : // mult 3
                        phase = g_Modulation_Phase[source_addresse] * 3.;
                        g_Modulation_Reset[LFO1_OUT] = (phase < g_Modulation_Phase[source_addresse]);
                    break;
                    case 7 : // mult 4
                        phase = g_Modulation_Phase[source_addresse] * 4.;
                        g_Modulation_Reset[LFO1_OUT] = (phase < g_Modulation_Phase[source_addresse]);
                    break;
                    case 8 : // mult 8
                        phase = g_Modulation_Phase[source_addresse] * 8.;
                        g_Modulation_Reset[LFO1_OUT] = (phase < g_Modulation_Phase[source_addresse]);
                    break;
                }
                phase = wrap(phase + mix_factor);
                g_Modulation_Phase[LFO1_OUT] = phase;
                modulation = LFO_compute_WF(phase, curent_config.c_LFO1_WF, g_LFO1_noise, g_Modulation_Reset[LFO1_OUT]);
                break;
            } else {//automodulation donc autre algo : on change le PWM
                WF1 = mix_factor + mix_factor - 1.;

                phase = g_Modulation_Phase[LFO1_OUT] + increment;                               // calcul de la phase
                overflow_phase = (int)phase;
                phase -= overflow_phase;
                g_Modulation_Reset[LFO1_OUT] = overflow_phase;
                g_Modulation_Phase[LFO1_OUT] = phase;

                if (WF1 > 0.) {
                    phase *= mix(1., phase, WF1);
                    phase *= mix(1., phase, WF1);
                    phase *= mix(1., phase, WF1);
                    phase *= mix(1., phase, WF1);
                }
                else {
                    WF1 = -WF1;
                    phase = 1. - phase;
                    phase *= mix(1., phase, WF1);
                    phase *= mix(1., phase, WF1);
                    phase *= mix(1., phase, WF1);
                    phase *= mix(1., phase, WF1);
                    phase = 1. - phase;
                }
                modulation = LFO_compute_WF(phase, curent_config.c_LFO1_WF, g_LFO1_noise, g_Modulation_Reset[LFO1_OUT]);
                break;
            }
        case LFO_GATE : // ok
            {
                double l_mix_factor;
                if (source_addresse != NONE_OUT) { //automodulation
                    WF1 = g_Modulation[source_addresse];
                    l_mix_factor = mix_factor;
                } else {
                    WF1 = 0.;
                    l_mix_factor = 1.-mix_factor;
                }
                phase = g_Modulation_Phase[LFO1_OUT] + increment;                               // calcul de la phase
                overflow_phase = (int)phase;
                phase -= overflow_phase;
                g_Modulation_Reset[LFO1_OUT] = overflow_phase;
                g_Modulation_Phase[LFO1_OUT] = phase;
                if (WF1 > ( (2.*l_mix_factor)-1.) ) {
                    modulation = LFO_compute_WF(phase, curent_config.c_LFO1_WF, g_LFO1_noise, g_Modulation_Reset[LFO1_OUT]);
                } else modulation = g_Modulation[LFO1_OUT];
            }
            break;
        case LFO_TH : // ok
            {
                double l_mix_factor;
                if (source_addresse != NONE_OUT) { //automodulation
                    WF1 = g_Modulation[source_addresse];
                    l_mix_factor = mix_factor;;
                } else {
                    WF1 = 0.;
                    l_mix_factor = 1.-mix_factor;;
                }
                phase = g_Modulation_Phase[LFO1_OUT] + increment;                               // calcul de la phase
                overflow_phase = (int)phase;
                phase -= overflow_phase;
                g_Modulation_Reset[LFO1_OUT] = overflow_phase;
                g_Modulation_Phase[LFO1_OUT] = phase;

                modulation = g_Modulation[LFO1_OUT];
                if (g_LFO1_last_step == 0) {
                    if ( WF1 > 0.01 + ((2.*l_mix_factor)-1.) )
                    {
                        modulation = LFO_compute_WF(phase, curent_config.c_LFO1_WF, g_LFO1_noise, g_Modulation_Reset[LFO1_OUT]);
                        g_LFO1_last_step = 1;
                    }
                }
                else {
                    if ( WF1 < -0.01 + ((2.*l_mix_factor)-1.) )
                    {
                        g_LFO1_last_step = 0;
                    }
                }
            }
            break;
        case LFO_RndLoop : // fq = nb de step, mix_factor = variation speed
        // sans modulation,et en automod : 'mod' permet de passer les step, mais attention au nb de step
            {
                uint32_t WF1_reset;
                double l_mix_factor;
                if ( (source_addresse != NONE_OUT) || (source_addresse != LFO1_OUT) ) {
                    WF1_reset = g_Modulation_Reset[source_addresse];
                    l_mix_factor = mix_factor;
                } else { //automodulation ou pas de modulation
                    if (g_LFO1_reset == false) {
                        if (mix_factor > 0.51) {
                            WF1_reset = true;
                            g_LFO1_reset = true;
                        } else {
                            WF1_reset = false;
                        }
                    } else {
                        WF1_reset = false;
                        if (mix_factor < 0.49) {
                            g_LFO1_reset = false;
                        }
                    }
                    l_mix_factor = 1.;
                }
                {   uint32_t nb_step = table_step[(int)(l_mix_factor*5.9999)];
                    if (WF1_reset) {
                        if(++g_LFO1_last_step >= (uint)nb_step) g_LFO1_last_step=0;
                        uint32_t num_thomas = thomas_LFO1 + 3*g_LFO1_last_step;
                        thomas(num_thomas, fq*fq, 0.2);
                        g_LFO1_noise[1] = g_LFO1_noise[0];
                        g_LFO1_noise[0] = g_thomasX[num_thomas];
                        g_Modulation_Reset[LFO1_OUT] = 1;

                    } else {
                        g_Modulation_Reset[LFO1_OUT] = 0;
                    }
                    modulation = Interpolation_Curve(g_Modulation_Phase[source_addresse], curent_config.c_LFO1_WF, g_LFO1_noise);
                    g_Modulation_Phase[LFO1_OUT] = g_Modulation_Phase[source_addresse];
                }
            }
            break;
        }
        g_LFO1_AR[0]= modulation; // sauvegarde pour ne pas avoir a tester la WF du LFO ds la boucle audio
        g_LFO1_AR[1]= modulation;
        g_LFO1_AR[2]= modulation;
        g_LFO1_AR[3]= modulation;
        g_LFO1_AR[4]= modulation; // la somme de tout les AR = LFO en mode normal
        g_Modulation[LFO1_OUT] = modulation;
        g_Modulation[LFO1_OUT+modulation_source_last] = -modulation;
        g_Modulation[LFO1_OUT_FILTER] += 0.003 * (modulation - g_Modulation[LFO1_OUT_FILTER]); // pour les leds
    }
}

double g_LFO4_noise[4]; // pour le noise avec interpolation cubic

inline void LFO4(double increment) {
    double phase, overflow_phase, tmp;

    phase = g_Modulation_Phase[LFO4_OUT] + increment;                               // calcul de la phase
    overflow_phase = (int)phase;
    phase -= overflow_phase;
    g_Modulation_Reset[LFO4_OUT] = overflow_phase;
    g_Modulation_Phase[LFO4_OUT] = phase;

    tmp = LFO_compute_WF(phase, curent_config.c_LFO4_WF, g_LFO4_noise, g_Modulation_Reset[LFO4_OUT]);
    g_Modulation[LFO4_OUT] = tmp;
    g_Modulation[LFO4_OUT+modulation_source_last] = -tmp;
    g_Modulation[LFO4_OUT_FILTER] += 0.003 * (tmp - g_Modulation[LFO4_OUT_FILTER]);
}
